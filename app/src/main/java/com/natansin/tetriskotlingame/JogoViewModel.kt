...
import com.natansin.tetriskotlingame.Spirit.Companion.Empty
...
class JogoViewModel : ViewModel() {
    ...
    private suspend fun clearScreen(state: ViewState): ViewState {
        ...
        (0..state.matrix.second).forEach { y ->
            emit(
                state.copy(
                    gameStatus = GameStatus.ScreenClearing,
                    bricks = Bloco.of(xRange, y until state.matrix.second),
                    spirit = Empty
                ).also { newState = it }
            )
            delay(50)
    ...
    private fun updateBricks(
        curBricks: List<Bloco>,
        spirit: Spirit,
        matrix: Pair<Int, Int>
    ): Pair<Triple<List<Bloco>, List<Bloco>, List<Bloco>>, Int> {
        val bricks = (curBricks + Bloco.of(spirit))
    ...
    data class ViewState(
        val bricks: List<Bloco> = emptyList(),
